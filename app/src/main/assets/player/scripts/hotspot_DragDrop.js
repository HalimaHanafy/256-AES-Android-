...
$(function(){
    ...
    for(var x=0; x<jsonData.length; x++){
        $('.drop_container').append('<a style="animation-delay:1s" id="'+jsonData[x].num+'" href="#" class="animated bounceIn drop_item drop'+x+'"></a>');
        $('#'+jsonData[x].num).css('top',jsonData[x].top+'px');
        $('#'+jsonData[x].num).css('left',jsonData[x].left+'px');
        $('.Drag_container').append('<a style="animation-delay:1s" href="#" class="animated bounceIn Drag_item">'+jsonData[x].answer+'</a>');
    }
